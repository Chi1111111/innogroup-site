...
export async function openCloudInventory({token,request=fetch}={}) {
  ...
  return {index,read,
    ...
    async publish(files){
      // A concurrent website/collector commit must never be overwritten.
      if((await api('git/ref/heads/main')).object.sha!==head)throw new Error('Cloud inventory changed during scan; upload stopped to avoid overwriting newer data. Run again.');
      const nextTree=await api('git/trees','POST',{base_tree:commit.tree.sha,tree:[...files].map(([name,value])=>({path:`public/data/japan-market/${name}`,mode:'100644',type:'blob',content:JSON.stringify(value)}))});
      const nextCommit=await api('git/commits','POST',{message:'Update Japan Market from memory-only local scan',tree:nextTree.sha,parents:[head]});
