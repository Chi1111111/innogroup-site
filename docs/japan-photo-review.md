...
Source access diagnosis: GitHub's first run received 39 source HTTP 403 responses from www.japancars.co.jp; the same two sampled URLs downloaded successfully with the same code locally. Queue processing is paused. The worker now pauses automatically on source HTTP 401/403 rather than continuing through the queue. Local pre-upload followed by cloud-only processing remains a proposed next step, not yet implemented. Admin progress uses a private lightweight status endpoint every ten seconds without refreshing image selection.
