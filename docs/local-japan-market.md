...
关闭网页不会停止采集，关闭本地程序会中断当前请求，下次启动恢复云端断点。完成后等网站发布，再刷新 Admin 运行记录。一次仅在一台电脑扫描，避免不同电脑或云端任务同时更新造成上传冲突。本功能不注册开机服务；安装启动入口后，可从 Admin 唤起尚未运行的程序。
...
单个车型目录连续三辆均缺少库存编号、且该目录本次没有成功读取详情时，暂跳该车型到下一轮，同一来源会话检查下一车型。若下一车型仍缺失则结束本批；正常详情会恢复计数。该暂跳位置写入云端断点，不把失败车辆计为已采集。明确 HTTP 限制与挑战仍直接停止。

日文车型目录若返回末尾缺少车型 slug 的详情链接（如 chevrolet--库存编码.html），作为 missing_model_slug 记录并跳过详情请求，继续后续目录。正常链接的日文车型不受影响；跳过不计入新增，也不会永久加入已采车辆集合，下轮可重新检查链接是否修复。

## 网页唤起（Windows）
解压后双击 Install-Launcher.cmd，一次性为当前 Windows 用户注册 innogroup-collector 启动入口，无需管理员权限。目录移动后需重新注册。在 Admin 点“唤起本地采集程序”，允许浏览器打开程序；复制控制台配对码后开始扫描。启动入口只启动固定程序，不接受 URL 中的命令，也不自动开始采集。浏览器可能始终要求确认，无法保证无提示启动。服务已运行时不会再开一个实例。
GitHub 每日任务与 Admin 线上手动扫描现在使用相同的云端断点、自动批次及逐车变更报告。两种运行方式共享断点，只同时运行一种。GitHub 机房仍可能被来源站拒绝访问；修改采集逻辑不能保证消除 HTTP 403。线上运行中可查看 GitHub 日志，完成并发布后在 Admin 查看逐车更新详情与结束原因。

## 稳定性与恢复
启动后优先将云端断点内的已验证车辆入库，成功后再开始新批次。临时云端读取故障最多尝试三次；入库最多尝试三次，重试前重新读取当前库存，只补入仍不存在的车辆，保留其他任务已入库的数据。明确权限错误、长期额度限制及真实断点冲突仍停止，不能无限重试。入库错误及重试次数会写入运行详情。
同一车型组合连续累计五次车型不匹配且没有新增时，暂跳该目录并保存匹配样例；不会把不匹配车辆强行改名入库。后续轮次仍可重新检查目录。

断点现在按需每 60 秒合并保存一次，批次入库前后强制保存，减少 GitHub 写入压力。强制关机时最近约一分钟未确认的数据可能重新读取；以 Admin 的“云端断点”时间为准，不会跳过该断点之后未保存的车辆。GitHub 错误会记录失败接口和服务端响应，不将所有 422 一概当作网络错误重试。
