...
## 品牌／车型轮询（当前默认）
使用源站公开品牌、车型目录及 make + maker 筛选，每个品牌和车型组合每轮最多采集 5 辆未入库、校验通过的车辆。不足 5 辆会转到下一组合；全部组合完成后进入下一轮，整轮没有新增则结束。一批中未处理的候选车不会提前标为已读。自动续批在内存保留当前品牌、车型、轮次和该组合已采数量；手动重新启动服务后从目录起点重新轮询，已上传车辆继续跳过。累计仍最多新增 5,000 辆，总时长与批次数上限不变。车型很多时可能在完成一整轮之前达到数量或时间上限。此策略用于均匀覆盖车型，不能保证绕过源站访问限制。
