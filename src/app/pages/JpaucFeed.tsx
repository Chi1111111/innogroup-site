...
const NZD_TO_JPY_RATE = 86.5;
const JAPAN_BASE_FEE_JPY = 100000;
const IMPORT_MARKUP = 1.15;
const FIXED_NZD_FEES = 2100 + 1000 + 500;
const SERVICE_MARKUP = 1.05;
...
function calculateEstimatedNzd(jpyPrice: number | null) {
  if (!jpyPrice || jpyPrice <= 0) return null;
  return ((jpyPrice + JAPAN_BASE_FEE_JPY) / NZD_TO_JPY_RATE * IMPORT_MARKUP + FIXED_NZD_FEES) * SERVICE_MARKUP;
}

function formatEstimatedNzd(jpyPrice: number | null) {
  const estimated = calculateEstimatedNzd(jpyPrice);
  if (!estimated) return 'POA';

  return `$${Math.round(estimated).toLocaleString('en-NZ')} NZD`;
}

function formatJpyPrice(value: string) {
  const amount = parsePriceNumber(value);
  if (!amount) return value || 'N/A';
...
function matchesFilters(item: SearchIndexEntry, filters: FeedFilters) {
  ...
  const estimatedNzd = calculateEstimatedNzd(parsePriceNumber(item.startPrice));
  if (priceFrom !== null && estimatedNzd !== null && estimatedNzd < priceFrom) return false;
  if (priceTo !== null && estimatedNzd !== null && estimatedNzd > priceTo) return false;
            ...
            <div className="space-y-3 p-4">
              ...
              <div className="grid gap-3 md:grid-cols-[1fr_1fr_auto_auto]">
                <input
                  ...
                  placeholder="Estimated NZD From"
                  className="rounded-lg border border-black/12 bg-white px-3 py-2.5 text-sm outline-none focus:border-primary"
                />
                <input
                  type="number"
          ...
          {loading || (pageLoading && visibleVehicles.length === 0) ? (
            ...
            <div className="space-y-5">
              ...
              {visibleVehicles.map((vehicle) => {
                ...
                const estimatedNzdPrice = formatEstimatedNzd(parsePriceNumber(vehicle.startPrice));
                const japanPrice = formatJpyPrice(vehicle.startPrice);
                const priceSourceLabel = activeFeed === 'auction' ? 'Japan start price' : 'Japan price';
                const estimatedPriceLabel =
                  activeFeed === 'auction' ? 'Estimated NZD start price' : 'Estimated landed price';
                const enquiryMessage = [
                  "Hi Inno Group, I'm interested in this vehicle:",
                  vehicleTitle,
                  `Type: ${activeFeed === 'auction' ? 'Auction' : 'One Price (Japan)'}`,
                  `ID: ${vehicle.id}`,
                  `${estimatedPriceLabel}: ${estimatedNzdPrice}`,
                      ...
                      <div className="space-y-4">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          ...
                          <div className="rounded-xl border border-primary/30 bg-primary/10 px-3 py-2.5 text-right">
                            ...
                            <p className="mt-1 text-4xl font-bold leading-none text-primary md:text-5xl">
                              {estimatedNzdPrice}
                            </p>
