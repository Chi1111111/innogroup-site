...
export function JapanMarket({ initialMakeSlug = '', initialModelSlug = '' }: { initialMakeSlug?: string; initialModelSlug?: string }) {
  ...
  return <main className="japan-market">
    <section className="jm-search-header"><div className="jm-shell">
      ...
      <div className="jm-heading-row"><div><p className="jm-eyebrow">JAPAN → NEW ZEALAND</p><h1>{text({ en: 'Find your car in Japan.', zh: '日本好车，直接找。' })}</h1></div><p className="jm-source-note">{text({ en: 'Japan inventory. FOB quotes and estimates in NZD.', zh: '日本车源 · FOB 报价及参考价' })}</p></div>
    ...
    <div className="jm-shell jm-workspace">
      ...
      <div className="jm-results" ref={results}>
        ...
        <details className="jm-about-data"><summary>{text({ en: 'About these listings & FOB prices', zh: '车源及 FOB 报价说明' })}</summary><p>{text({ en: 'Japan Cars and retained Carsensor inventory. Estimated FOB prices for retained stock are converted from recorded source vehicle prices, not confirmed exporter FOB quotes. Freight, insurance, New Zealand taxes, compliance, registration and Inno services are quoted separately. Availability and prices are reconfirmed before purchase.', zh: '车源包含 Japan Cars 及保留的 Carsensor 库存。旧库存的 FOB 参考价由记录的车辆源价格换算，并非已确认的出口商 FOB 报价。运费、保险、新西兰税费、合规、注册和 Inno 服务费另计。购买前会再次确认库存及报价。' })}</p></details>
