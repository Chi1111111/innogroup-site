...
export function JapanMarket({ initialMakeSlug = '', initialModelSlug = '' }: { initialMakeSlug?: string; initialModelSlug?: string }) {
  ...
  return <main className="japan-market">
    <section className="jm-search-header"><div className="jm-shell">
      ...
      <div className="jm-heading-row"><div><p className="jm-eyebrow">JAPAN → NEW ZEALAND</p><h1>{text({ en: 'Find your car in Japan.', zh: '日本好车，直接找。' })}</h1></div><p className="jm-source-note">{text({ en: 'Japan Cars listings. FOB prices in NZD.', zh: 'Japan Cars 日本车源 · FOB 纽币报价' })}</p></div>
    ...
    <div className="jm-shell jm-workspace">
      ...
      <div className="jm-results" ref={results}>
        ...
        <details className="jm-about-data"><summary>{text({ en: 'About these listings & FOB prices', zh: '车源及 FOB 报价说明' })}</summary><p>{text({ en: 'Japan Cars dealer listings with FOB prices in NZD. Freight, insurance, New Zealand taxes, compliance, registration and Inno services are quoted separately. Availability and prices are reconfirmed before purchase.', zh: '车源来自 Japan Cars，FOB 离岸价以纽币展示。运费、保险、新西兰税费、合规、注册和 Inno 服务费另计。购买前会再次确认库存及报价。' })}</p></details>
