...
export function AdminPhotos(){
 ...
 const count=data?.counts.pending_review||0;
 ...
 return <section className="photo-admin"><h1>图片处理与自动上架</h1><p>本地单线程处理已有库存，每张完成后间隔 2 秒再下载下一张。照片仅在内存中处理并上传 Supabase，不保存到本地硬盘；关机后暂停，重新启动后续跑。不调用大模型。处理后的照片自动上传；未检测到已知 JAPANCARS 水印的照片自动通过，这里仅展示明确检测到水印的照片；不确定的照片暂存，仍会阻止对应车辆上架。车辆全部照片通过且文件校验成功后自动上架。</p>
 ...
 <div className="photo-controls"><span>仅显示已检测到水印的照片</span><select aria-label="每页数量" disabled={busy} value={size} onChange={e=>{setSize(Number(e.target.value));setOffset(0);}}>{[20,50,100].map(n=><option key={n} value={n}>每页 {n} 张</option>)}</select></div>
 ...
 {!busy&&data?.items.length===0&&<p>当前没有检测到水印的待审核照片。未检测到已知水印且符合规则的照片自动通过；不确定的照片暂存，不会自动上架。</p>}
