...
const filterCount=(counts:Record<string,number>,filter:string)=>filter==='awaiting_review'?(counts.pending_review||0)+(counts.needs_inspection||0):filter?(counts[filter]||0):Object.values(counts).reduce((a,b)=>a+b,0);
const api=<T,>(action:string,body:Record<string,unknown>={})=>invokeAdminFunction<T>('japan-photo-review',{action,...body});
export function AdminPhotos(){
 const [data,setData]=useState<Listing|null>(null),[filter,setFilter]=useState('awaiting_review'),[size,setSize]=useState(20),[offset,setOffset]=useState(0),[selected,setSelected]=useState<Set<string>>(new Set()),[busy,setBusy]=useState(false),[error,setError]=useState(''),[notice,setNotice]=useState(''),[zoom,setZoom]=useState<Photo|null>(null),[decision,setDecision]=useState<string|null>(null),[ack,setAck]=useState(false);
 ...
 const count=filterCount(data?.counts||{},filter);
 ...
 return <section className="photo-admin"><h1>图片处理与自动上架</h1><p>本地单线程处理已有库存，每张完成后间隔 2 秒再下载下一张。照片仅在内存中处理并上传 Supabase，不保存到本地硬盘；关机后暂停，重新启动后续跑。不调用大模型。处理后的照片自动上传；未检测到已知 JAPANCARS 水印的照片自动通过，检测到水印或结果不确定的留在这里审核。车辆全部照片通过且文件校验成功后自动上架。</p>
 ...
 <div className="photo-controls"><select aria-label="图片状态" disabled={busy} value={filter} onChange={e=>{setFilter(e.target.value);setOffset(0);}}><option value="awaiting_review">需人工审核（{filterCount(data?.counts||{},'awaiting_review')}）</option>{Object.entries(labels).filter(([key])=>['pending_review','needs_inspection'].includes(key)).map(([key,label])=><option key={key} value={key}>{label}（{data?.counts[key]||0}）</option>)}</select><select aria-label="每页数量" disabled={busy} value={size} onChange={e=>{setSize(Number(e.target.value));setOffset(0);}}>{[20,50,100].map(n=><option key={n} value={n}>每页 {n} 张</option>)}</select></div>
 ...
 {!busy&&data?.items.length===0&&<div><p>{filter==='awaiting_review'?'当前没有待审核照片；处理完成后请刷新照片列表。':'当前筛选下没有照片，不代表没有处理结果。'}</p>{filter!=='awaiting_review'&&filterCount(data?.counts||{},'awaiting_review')>0&&<button onClick={()=>{setFilter('awaiting_review');setOffset(0);}}>查看需人工审核（{filterCount(data?.counts||{},'awaiting_review')}）</button>}</div>}
