...
export function AdminPhotos(){
 ...
 return <section className="photo-admin"><h1>图片处理与自动上架</h1><p>本地流水线处理已有库存，同时最多处理 2 张；来源下载保持单路，请求开始时间至少间隔 1.5 秒。照片仅在内存中处理并上传 Supabase，不保存到本地硬盘；关机后暂停，重新启动后续跑。不调用大模型。处理后的照片自动上传；未检测到水印的只存成品，检测到水印的保留原图供审核。未检测到已知 JAPANCARS 水印的照片自动通过，原先的“疑似”结果也按此规则自动通过，这里仅展示明确检测到水印的照片。车辆全部照片通过且文件校验成功后自动上架。</p>
 ...
 {!busy&&data?.items.length===0&&<p>当前没有明确检测到水印的待审核照片；其他照片上传校验成功后自动通过。</p>}
