    ...
    <div className="admin-workspace">
      ...
      <div className="admin-body">
        <header className="admin-topbar">
          <button type="button" className="admin-menu-toggle" onClick={() => setMenuOpen(!menuOpen)} aria-label={menuOpen ? '关闭导航' : '展开导航'} aria-expanded={menuOpen} aria-controls="admin-sidebar">{menuOpen ? <X size={20} /> : <Menu size={20} />}</button>
        ...
        <div key={pathname} id="admin-content" className="admin-content" tabIndex={-1}><Outlet /></div>
      </div>
    </div>
  );
}
